...
Tailscale 経由で instance に到達し，Tailscale SSH で `git pull` → `docker compose up -d --build` → health check を行う．
SSH 秘密鍵は GitHub に置かず，認証は Tailscale SSH と ACL が担う．

必要な設定は次のとおりである．

- Secrets：`TS_OAUTH_CLIENT_ID` / `TS_OAUTH_SECRET`（scope: devices，`tag:ci` 付与）．
- Variables：`DEPLOY_HOST`（MagicDNS 名）/ `DEPLOY_USER` / `DEPLOY_PATH`．
